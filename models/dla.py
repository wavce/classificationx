...
def _print(x):
    tf.print(x.name, tf.transpose(x, [0, 3, 1, 2]))

    return x

_Print = tf.keras.layers.Lambda(lambda x: _print(x))
...
def tree(inputs,
    ...
    if strides > 1:
        bottom = tf.keras.layers.MaxPool2D(strides, strides, name=name + "/downsample")(inputs)
    else:
        bottom = inputs
...
class DLA(Backbone):
    ...
    def build_model(self):
        ...
        if self.return_levels:
            x = tf.keras.layers.AvgPool2D((7, 7))(x)
            outputs = tf.keras.layers.Conv2D(self.num_classes, kernel_size=1, strides=1, name="fc")(x)
...
def _torch2h5(model, torch_weights_path, output_path):
    import re
    import torch
    import numpy as np
...
if __name__ == "__main__":
    name = "dla34"
    model = DLA34(input_shape=(512, 512, 3), output_indices=(-1, ))
    _torch2h5(model, 
             "/home/bail/Workspace/pretrained_weights/%s.pth" % name, 
             "/home/bail/Workspace/pretrained_weights/%s.h5" % name)
